...
const userList = async (req, res, next) => {
  try {
    ...
    const usersWithLanguages = users.map((user) => {
      const languageNames = user.languages
        .map((languageId) => {
          const languageName = languageMap[languageId.toString()];
          return languageName ? languageName : undefined;
        })
        .filter((name) => name !== undefined); // Removing undefined values from the array
    ...
    res.render("users", { users: usersWithLanguages });
  } catch (error) {
    console.error(error);
    next(error);
  }
